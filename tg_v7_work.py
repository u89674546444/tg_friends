...
async def choose_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_choice == "Добавить фото выполненной работы":
        logger.info("Переход к состоянию RECEIVING_PHOTO_AFTER.")
        ...
        # Получаем путь к текущему отчету
        report_dir = context.user_data.get("report_dir")
        if not report_dir:
            logger.error("Ошибка: папка отчета не найдена.")
            await update.message.reply_text("Ошибка: папка отчета не найдена.")
            return ConversationHandler.END
        ...
        try:
            ...
            logger.info(f"Статус задачи обновлен на 'не выполнено' в файле: {text_file_path}")
        except Exception as e:
            logger.error(f"Ошибка при обновлении файла {text_file_path}: {e}")
        ...
        if not unfinished_tasks:
            logger.info("Нет невыполненных задач.")
            await update.message.reply_text("Нет невыполненных задач. Начните новую задачу.")
            return SELECTING_HOUSE
        ...
        logger.info(f"Список невыполненных задач: {response}")
        await update.message.reply_text(response)
        await update.message.reply_text("Введите номер задачи для продолжения:")
...
#Добавьте обработчик для состояния RECEIVING_TASK_NUMBER, чтобы бот мог продолжить работу с выбранной задачей:
async def receive_task_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("Пожалуйста, введите число.")
        return RECEIVING_TASK_NUMBER
#Эта функция ищет все невыполненные задачи в папке reports.
def get_unfinished_tasks():
    base_dir = "reports"
    unfinished_tasks = []

    # Логирование для проверки, что функция вызывается
                    ...
                    with open(file_path, "r", encoding="utf-8") as f:
                        ...
                        if "не выполнено" in content:
                            ...
                            if len(lines) >= 2:
                                ...
                                logger.warning(f"Файл {file_path} содержит недостаточно строк.")
                        else:
